...
data class SuffixArray(val s1: String, val s2: String) {
    val ALPHABET_SIZE = 31
    val NEUTRAL_CHAR = 'a'.toInt() - 1
    val s1Len = s1.length

    val s = s1 + ('z'.toInt() + 1).toChar() + s2 + NEUTRAL_CHAR.toChar()
    val length = s.length
    ...
    fun getCommonLine() : String {
        ...
        for (i in 1 until length - 1) {
            if ((suffixes[i + 1] < s1Len && suffixes[i] > s1Len) ||
                    (suffixes[i + 1] > s1Len && suffixes[i] < s1Len)) {
        ...
        return s.substring(suffixes[currLineIndex], suffixes[currLineIndex] + maxLCP)
    }
...
fun main(args: Array<String>) {
    val scanner = FastReader("common.in")

    val sa = SuffixArray(scanner.next(), scanner.next())

    val writer = FastWriter("common.out")

    writer.println(sa.getCommonLine())
    writer.close()
}
