...
data class SuffixArray(val s: String) {
    ...
    var numberOfDifferentClasses = 0

    fun getShift(number: Int) : String {
        if (number > numberOfDifferentClasses) {
            return "IMPOSSIBLE"
        } else {
            val numberOfShift = length / numberOfDifferentClasses
            return s.substring(suffixes[(number - 1) * numberOfShift]) + s.substring(0, suffixes[(number - 1) * numberOfShift])
        }
    }
    ...
    fun printlnSuffixArray() {
        ...
        for (i in 0 until length) {
            for (index in suffixes[i] until length) {
                print(s[index])
            }
    ...
    init {
        ...
        numberOfDifferentClasses = currClass
...
fun main(args: Array<String>) {
    val scanner = FastReader("shifts.in")

    val sa = SuffixArray(scanner.next())
//    sa.printlnSuffixArray()

    val writer = FastWriter("shifts.out")
    writer.println(sa.getShift(scanner.nextInt()))
    writer.close()
}
